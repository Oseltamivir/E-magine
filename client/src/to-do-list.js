...
class Todo extends Component {
    ...
    upvoteAnswer = (key) => {
        ...
        alert(newAnswer)
        this.state.answerItems[ind] = newAnswer
    }
    upvoteReply = (key) => {
    ...
    addItem = (ev) => { ev.preventDefault(); let newState = { key: Date.now() + this.props.user, text: this.state.currentText, user: this.props.user, counter: -1 }; if (newState.text !== '') { let item = [...this.state.items, newState]; this.setState({ currentState: newState, items: item, currentText: '' }) } else { alert('Wrong Input') } }
    handleItem = (ev) => { this.setState({ currentText: ev.target.value }) }
    addAnswer = (ev) => { ev.preventDefault(); let newState = { key: Date.now() + this.props.user, text: this.state.currentText, user: this.props.user, counter: -1 }; if (newState.text !== '') { let item = [...this.state.answerItems, newState]; this.setState({ currentAnswer: newState, answerItems: item, currentText: '' }) } else { alert('Wrong Input') } }
    render() {
        return (
            <div className='todolist' >
                <form onSubmit={this.addItem}>
                    <Listmaker answers={this.state.answerItems} entries={this.state.items} deleteItem={this.deleteItem.bind(this)} user={this.props.user} deleteAnswer={this.deleteAnswer.bind(this)} upvoteAnswer={this.upvoteAnswer.bind(this)} upvoteReply = {this.upvoteReply.bind(this)}/>
                    <div id='Postbar'>

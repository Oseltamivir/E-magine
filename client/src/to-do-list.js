...
deleteItem = (key) => {const filteredItems = this.state.items.filter(item => {return item.key !== key});this.setState({items:filteredItems})}
  ...
  addAnswer =(ev) => { ev.preventDefault();let newState = {key:Date.now()+this.props.user, text:this.state.currentText,user:this.props.user};if(newState.text !== ''){let item = [...this.state.items, newState];this.setState({currentAnswer:newState,answerItems:item,currentText:''})}else{alert('Wrong Input')}}
    render(){
        return(
            <div className = 'todolist' >
               <form onSubmit = {this.addItem}>
                    <Listmaker answers = {this.state.answerItems} entries = {this.state.items} deleteItem = {this.deleteItem.bind(this)} user = {this.props.user}/>
                    <div id = 'Postbar'>
